...
export const addCartItem = (data) => async (dispatch) => {
  ...
  try {
    const { response } = await api.post('/cart/add', response.data);
...
export const RemoveCartItem = (data) => async (dispatch) => {
  ...
  try {
    const { response } = await api.delete(`/cartItems/${response.cartItemId}`);
...
export const UpdateCartItem = (data) => async (dispatch) => {
  ...
  try {
    const { data } = await api.put(`/cartItems/${data.cartItemId}`, data.data);
